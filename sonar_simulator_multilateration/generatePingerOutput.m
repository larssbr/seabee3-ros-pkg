...
time = 0:sample_period:pinger_period;

%Arbitrarily begin ping at 1/2 pinger_period second mark
ping_time = uint32(length(time)/2):uint32(length(time)/2 + ping_duration/sample_period);

signal = zeros(1, length(time));

signal(ping_time) = sin(2*pi*pinger_frequency*double(ping_time)*sample_period);
